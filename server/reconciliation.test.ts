...
import { calculateReconciliation, detectDiscrepancies, getAuditRisk } from "./reconciliation";
...
describe("detectDiscrepancies", () => {
  const products = [{ id: 1, metrcPackageId: "pkg-1", productName: "Flower", sku: "flower", quantity: 100, testingStatus: "Passed" }];

  it("compares the latest physical count and returns the persisted-ready finding", () => {
    const findings = detectDiscrepancies(products, new Map([["pkg-1", { quantity: 94 }]]), new Set());
    expect(findings[0]).toMatchObject({ product: products[0], physicalQuantity: 94, result: { varianceQuantity: -6, variancePercent: 6, severity: "medium" } });
  });

  it("suggests testing delay, damage logged, recent sale, and no count", () => {
    expect(calculateReconciliation({ metrcQuantity: 100, physicalQuantity: 90, testingStatus: "Pending", hasRecentDamage: false }).likelyCause).toBe("Testing delay");
    expect(calculateReconciliation({ metrcQuantity: 100, physicalQuantity: 90, testingStatus: "Passed", hasRecentDamage: true }).likelyCause).toBe("Damage logged");
    expect(calculateReconciliation({ metrcQuantity: 100, physicalQuantity: 90, testingStatus: "Passed", hasRecentDamage: false, hasRecentSale: true }).likelyCause).toBe("Recent sale");
    expect(calculateReconciliation({ metrcQuantity: 100, physicalQuantity: null, testingStatus: "Passed", hasRecentDamage: false }).likelyCause).toBe("No recent physical count");
  });
});
