...
import { countLogSchema, damageLogSchema, packageInputSchema, testLogSchema } from "../logValidation";
...
export const logsRouter = router({
  ...
  createCount: protectedProcedure.input(countLogSchema.omit({ type: true })).mutation(async ({ ctx, input }) => {
  ...
  createDamage: protectedProcedure.input(damageLogSchema).mutation(async ({ ctx, input }) => {
    const { facility, inventory } = await productForLog(ctx.user.id, input.metrcPackageId);
  ...
  createTest: protectedProcedure.input(testLogSchema.omit({ type: true })).mutation(async ({ ctx, input }) => {
    const { facility, inventory } = await productForLog(ctx.user.id, input.metrcPackageId);
