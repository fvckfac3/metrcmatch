...
export async function processInBatches<T>(
  ...
  for (let index = 0; index < items.length; index += batchSize) {
    await Promise.all(items.slice(index, index + batchSize).map(processor));
  }
}
...
export async function ensureFacilityForUser(userId: number) {
  ...
  const existing = await database
    ...
    .limit(1);
  if (existing[0])
    return { ...existing[0].facility, memberRole: existing[0].member.role };
...
export async function updateFacility(
  ...
  input: {
    ...
    address?: string | null;
    timezone: string;
    complianceManagerEmail?: string | null;
...
export async function getMetrcConnection(facilityId: number) {
  ...
  return (
    await database
      .select()
      .from(metrcConnections)
      .where(eq(metrcConnections.facilityId, facilityId))
      .limit(1)
  )[0];
}

export async function saveMetrcConnection(
  facilityId: number,
  input: {
  ...
  const values = {
    ...
    encryptedUserApiKey: input.userApiKey
      ? encryptSecret(input.userApiKey)
      : (current?.encryptedUserApiKey ?? null),
    encryptedIntegratorApiKey: input.integratorApiKey
      ? encryptSecret(input.integratorApiKey)
      : (current?.encryptedIntegratorApiKey ?? null),
    encryptedOauthClientId: input.oauthClientId
      ? encryptSecret(input.oauthClientId)
      : (current?.encryptedOauthClientId ?? null),
    encryptedOauthClientSecret: input.oauthClientSecret
      ? encryptSecret(input.oauthClientSecret)
      : (current?.encryptedOauthClientSecret ?? null),
  };
  if (current)
    await database
      .update(metrcConnections)
      .set(values)
      .where(eq(metrcConnections.id, current.id));
  else
    await database.insert(metrcConnections).values({ facilityId, ...values });
  return (
    await database
      .select()
      .from(metrcConnections)
...
export async function setConnectionStatus(
  ...
  options: { tested?: boolean; synced?: boolean } = {}
) {
  const database = await requireDb();
  await database
    .update(metrcConnections)
...
export async function getRunningSync(facilityId: number) {
  ...
  return (
    await database
      ...
      .where(
        and(
          eq(metrcSyncs.facilityId, facilityId),
          eq(metrcSyncs.status, "running")
        )
      )
      .orderBy(desc(metrcSyncs.startedAt))
      .limit(1)
  )[0];
}
...
export async function listSyncs(facilityId: number) {
  ...
  return database
    .select()
    .from(metrcSyncs)
    .where(eq(metrcSyncs.facilityId, facilityId))
    .orderBy(desc(metrcSyncs.startedAt))
    .limit(10);
}

export async function upsertInventorySnapshots(
  facilityId: number,
  records: Array<{
  ...
  await processInBatches(records, 25, async record => {
    await database
      .insert(inventorySnapshots)
      .values({ facilityId, ...record, quantity: String(record.quantity) })
...
export async function upsertMetrcTestResults(
  facilityId: number,
  records: Array<{
    metrcPackageId: string;
    testStatus: string;
  ...
  await processInBatches(records, 25, async record => {
    await database
      .insert(metrcTestResults)
      .values({ facilityId, ...record })
      .onDuplicateKeyUpdate({
        set: {
...
export async function listInventory(facilityId: number) {
  ...
  return database
    ...
    .where(eq(inventorySnapshots.facilityId, facilityId))
    .orderBy(inventorySnapshots.productName);
}

export async function getInventoryByPackage(
  facilityId: number,
  metrcPackageId: string
) {
  const database = await requireDb();
  return (
    await database
      ...
      .where(
        and(
          eq(inventorySnapshots.facilityId, facilityId),
          eq(inventorySnapshots.metrcPackageId, metrcPackageId)
        )
      )
      .limit(1)
  )[0];
...
export async function createPhysicalLog(input: {
  ...
  productName: string;
  sku?: string | null;
  type: "count" | "damage" | "discard" | "test_result";
  quantity?: number | null;
  location?: string | null;
  reason?: "broken" | "expired" | "theft" | "waste" | "other" | null;
  testStatus?: "passed" | "failed" | null;
  receivedAt?: Date | null;
  notes?: string | null;
}) {
  const database = await requireDb();
  const result = await database.insert(physicalLogs).values({
    ...input,
    quantity:
...
export async function listPhysicalLogs(
  ...
  if (query) {
    ...
    conditions.push(
      or(
        like(physicalLogs.productName, pattern),
        like(physicalLogs.sku, pattern),
        like(physicalLogs.location, pattern)
      )!
    );
  }
  return database
    ...
    .orderBy(desc(physicalLogs.occurredAt))
...
export async function latestLogsByPackage(facilityId: number) {
  ...
  for (const log of logs) {
    ...
    if (log.type === "count" && !latestCounts.has(log.metrcPackageId))
      latestCounts.set(log.metrcPackageId, log);
    if (log.type === "damage" || log.type === "discard")
      recentDamage.add(log.metrcPackageId);
  }
...
export async function getDiscrepancy(facilityId: number, packageId: string) {
  ...
  return (
    await database
      ...
      .where(
        and(
          eq(discrepancies.facilityId, facilityId),
          eq(discrepancies.metrcPackageId, packageId)
        )
      )
      .limit(1)
  )[0];
...
export async function upsertDiscrepancy(input: {
  facilityId: number;
  ...
  const values = {
    ...input,
    metrcQuantity: String(input.metrcQuantity),
    physicalQuantity:
      input.physicalQuantity === null ? null : String(input.physicalQuantity),
    varianceQuantity: String(input.varianceQuantity),
    variancePercent: String(input.variancePercent),
  };
  if (existing) {
    await database
      ...
      .set({
        ...values,
        status:
          existing.status === "resolved" ? "investigating" : existing.status,
        resolvedAt: null,
      })
...
export async function resolveClearedDiscrepancy(
  facilityId: number,
  packageId: string
) {
  ...
  await database
    ...
    .set({
      status: "resolved",
      resolutionNotes: "Variance cleared after reconciliation.",
      resolvedAt: new Date(),
    })
    .where(eq(discrepancies.id, existing.id));
}
...
export async function listDiscrepancies(
  facilityId: number,
  filters: {
    status?: "investigating" | "resolved" | "awaiting_lab" | "other";
    severity?: "critical" | "high" | "medium";
  } = {}
) {
  const database = await requireDb();
  ...
  if (filters.severity)
    conditions.push(eq(discrepancies.severity, filters.severity));
  return database
    .select()
    .from(discrepancies)
    .where(and(...conditions))
    .orderBy(
      sql`FIELD(${discrepancies.severity}, 'critical', 'high', 'medium')`,
      desc(discrepancies.detectedAt)
    );
}
...
export async function updateDiscrepancy(
  ...
  await database
    ...
    .where(
      and(eq(discrepancies.id, id), eq(discrepancies.facilityId, facilityId))
    );
}
...
export async function createNotification(input: {
  ...
  type: "critical_discrepancy" | "high_discrepancy" | "audit_risk_red";
  recipient?: string | null;
  detail: string;
  status?: "queued" | "sent" | "suppressed" | "failed";
}) {
  const database = await requireDb();
  const result = await database.insert(notificationEvents).values({
    ...input,
...
export async function getDashboardData(facilityId: number) {
  ...
  const [connection, inventory, allDiscrepancies, reports, syncHistory] =
    await Promise.all([
      ...
      database
        ...
        .where(eq(reconciliationReports.facilityId, facilityId))
      ...
      database
        ...
        .where(
          and(
            eq(metrcSyncs.facilityId, facilityId),
            eq(metrcSyncs.status, "success"),
            gte(metrcSyncs.startedAt, trendSince)
          )
        )
        .orderBy(metrcSyncs.startedAt),
    ]);
  ...
  const reconciled = await database
    ...
    .where(
      and(
        eq(physicalLogs.facilityId, facilityId),
        eq(physicalLogs.type, "count"),
        gte(physicalLogs.occurredAt, since)
      )
    );
  const trend = Array.from({ length: 7 }, (_, index) => {
    ...
    const sync = syncHistory.filter(
      item => item.startedAt.toISOString().slice(0, 10) === key
    );
    return {
      day: date.toLocaleDateString("en-US", { weekday: "short" }),
      count: sync.reduce((total, item) => total + item.inventoryItems, 0),
      syncs: sync.length,
    };
  });
  return {
    ...
    severities,
    reconciledThisWeek: Number(reconciled[0]?.count ?? 0),
  ...
  const [inventory, discrepancyRows] = await Promise.all([
    ...
    database
      ...
      .where(
        and(
          eq(discrepancies.facilityId, facilityId),
          gte(discrepancies.detectedAt, startDate),
          lte(discrepancies.detectedAt, endDate)
        )
      ),
  ]);
  const found = discrepancyRows.length;
  const resolved = discrepancyRows.filter(
    item => item.status === "resolved"
  ).length;
  const values = {
    facilityId,
    preparedByUserId: user.id,
    preparedByName: user.name ?? "Facility preparer",
    startDate,
    endDate,
    totalItemsReconciled: inventory.length,
    discrepanciesFound: found,
    discrepanciesResolved: resolved,
    outstandingDiscrepancies: found - resolved,
    criticalCount: discrepancyRows.filter(item => item.severity === "critical")
      .length,
    highCount: discrepancyRows.filter(item => item.severity === "high").length,
    mediumCount: discrepancyRows.filter(item => item.severity === "medium")
      .length,
  };
...
export async function getReportData(facilityId: number, reportId: number) {
  ...
  const report = (
    await database
      ...
      .where(
        and(
          eq(reconciliationReports.id, reportId),
          eq(reconciliationReports.facilityId, facilityId)
        )
      )
      .limit(1)
  )[0];
  ...
  const lines = await database
    ...
    .where(
      and(
        eq(discrepancies.facilityId, facilityId),
        gte(discrepancies.detectedAt, report.startDate),
        lte(discrepancies.detectedAt, report.endDate)
      )
    )
...
export async function getFacilityByScheduleTask(taskUid: string) {
  ...
  const connection = (
    await database
      .select()
      .from(metrcConnections)
      .where(eq(metrcConnections.scheduleCronTaskUid, taskUid))
      .limit(1)
  )[0];
  ...
  return {
    ...
    facility: (
      await database
        .select()
        .from(facilities)
        .where(eq(facilities.id, connection.facilityId))
        .limit(1)
    )[0]!,
  };
}

export async function setScheduleTask(
  facilityId: number,
  taskUid: string | null
) {
  const database = await requireDb();
