...
export default function Settings() {
  ...
  const connection = trpc.metrc.settings.useQuery();
  const history = trpc.metrc.history.useQuery();
  const [facilityName, setFacilityName] = useState("");
  const [licenseNumber, setLicenseNumber] = useState("");
  const [address, setAddress] = useState("");
  const [managerEmail, setManagerEmail] = useState("");
  const [authMethod, setAuthMethod] = useState<AuthMethod>("api_key");
  const [apiBaseUrl, setApiBaseUrl] = useState("https://api-or.metrc.com");
  const [metrcLicense, setMetrcLicense] = useState("");
